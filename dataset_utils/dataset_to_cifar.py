...
def process_image_dataset(source, destination, size = (32, 32), batch = 1):
    ...
    print("Total number of files %d" %(dataset_size))
        ...
        if(class_label != '.'): # Ignore source directory
            ...
            for counter, file in enumerate(files):
                ...
                if(REACHED_BATCH_MAX):
                    ...
                    print("Saved %d images for batch %d" %(counter, CURRENT_BATCH))
                    CURRENT_BATCH += 1
                    batch = open_batch(destination, CURRENT_BATCH)
                ...
                if (counter==0):
                      print("Image byte array shape %s" %(str(image_byte_array.shape)))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PyTorch dataset to cifar Evaluation')
    # dataset configs
    parser.add_argument('--src', type=str, help='The src folder for dataset')
    parser.add_argument('--dest', type=str, help='The dest folder for dataset')
    parser.add_argument('--batch', default=1, type=int, help='number of batches to split dataset')
    args = parser.parse_args()
    size = (32,32)
    process_image_dataset(args.src, args.dest, size, args.batch)